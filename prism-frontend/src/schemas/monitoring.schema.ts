...
export const monitoringKomponenSchema = z.object({
  ...
  planned_la: z.number().min(0, 'Rencana Loan Agreement tidak boleh negatif'),
  planned_usd: z.number().min(0, 'Rencana USD tidak boleh negatif'),
  planned_idr: z.number().min(0, 'Rencana IDR tidak boleh negatif'),
  realized_la: z.number().min(0, 'Realisasi Loan Agreement tidak boleh negatif'),
...
export const monitoringSchema = z.object({
  ...
  exchange_rate_la_idr: z.number().positive('Kurs Loan Agreement/IDR harus lebih dari 0'),
  planned_la: z.number().min(0, 'Rencana Loan Agreement tidak boleh negatif'),
  planned_usd: z.number().min(0, 'Rencana USD tidak boleh negatif'),
  planned_idr: z.number().min(0, 'Rencana IDR tidak boleh negatif'),
  realized_la: z.number().min(0, 'Realisasi Loan Agreement tidak boleh negatif'),
