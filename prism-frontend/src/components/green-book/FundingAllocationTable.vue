...
<template>
  <div class="overflow-x-auto rounded-lg border border-surface-200 bg-white">
    <table class="prism-table min-w-[64rem]">
      <thead class="bg-surface-50 text-xs font-semibold uppercase tracking-wide text-surface-500">
        <tr>
          ...
          <th class="prism-table-currency px-4 py-3">Jasa ({{ displayCurrency }})</th>
          <th class="prism-table-currency px-4 py-3">Konstruksi ({{ displayCurrency }})</th>
          <th class="prism-table-currency px-4 py-3">Goods ({{ displayCurrency }})</th>
          <th class="prism-table-currency px-4 py-3">Pelatihan ({{ displayCurrency }})</th>
          <th class="prism-table-currency px-4 py-3">Lainnya ({{ displayCurrency }})</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr v-if="activities.length === 0">
          <td colspan="6" class="prism-table-empty px-4 py-6">
      ...
      <tfoot>
        <tr>
          ...
          <td class="prism-table-currency px-4 py-2.5"><CurrencyDisplay :amount="totals.services" :currency="displayCurrency" /></td>
          <td class="prism-table-currency px-4 py-2.5"><CurrencyDisplay :amount="totals.constructions" :currency="displayCurrency" /></td>
          <td class="prism-table-currency px-4 py-2.5"><CurrencyDisplay :amount="totals.goods" :currency="displayCurrency" /></td>
          <td class="prism-table-currency px-4 py-2.5"><CurrencyDisplay :amount="totals.trainings" :currency="displayCurrency" /></td>
          <td class="prism-table-currency px-4 py-2.5"><CurrencyDisplay :amount="totals.other" :currency="displayCurrency" /></td>
        </tr>
      </tfoot>
    </table>
  </div>
